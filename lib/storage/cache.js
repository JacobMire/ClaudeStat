...
export async function getUsageCache() {
  try {
    ...
    // Stale Cache Handling - Check TTL and Resets
    const ageMs = Date.now() - (cache.fetchedAt || 0);
    const fiveHourResetMs = cache.fiveHourResetAt ? new Date(cache.fiveHourResetAt).getTime() : Infinity;
    const isPastReset = Date.now() >= fiveHourResetMs;

    if (ageMs > CACHE_TTL_MS || isPastReset) {
      logger.debug('Usage cache hit, but data is stale', { ageMs, isPastReset });
