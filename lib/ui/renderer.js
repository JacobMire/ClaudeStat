...
import { formatDuration, getMsRemaining, formatResetTime } from '../utils/time.js';
...
export function renderUsage(data, networkError = null, isUpdate = false, analytics = null) {
  ...
  const currentDayIndex = Math.max(0, Math.min(6, 6 - Math.floor(weeklyMs / (24 * 60 * 60 * 1000))));
  ...
  let html = `
    ...
    <div class="card ${anim1}">
      ...
      <div class="hero-sub">
        Resets in <strong class="model-timer" data-reset="${data.fiveHourResetAt || ''}" style="color: var(--text-primary);">${fiveHourStr}</strong> at <strong style="color: var(--text-primary);">${formatResetTime(data.fiveHourResetAt)}</strong>
      </div>
    ...
    <div class="card ${anim2}" style="margin-top: 16px;">
      ...
      <div class="hero-sub">
        Resets in <strong class="model-timer" data-reset="${data.weeklyResetAt || ''}" style="color: var(--text-primary);">${weeklyStr}</strong> at <strong style="color: var(--text-primary);">${formatResetTime(data.weeklyResetAt)}</strong>
      </div>
      
      <div style="position: relative; margin-top: 16px;">
        <div class="track track-weekly" style="margin-top: 0;">
        ...
        <div class="weekly-days-row">
          ${[0, 1, 2, 3, 4, 5, 6].map(i => {
            const d = new Date(Date.now() - (currentDayIndex - i) * 24 * 60 * 60 * 1000);
            const dayStr = d.toLocaleDateString(undefined, { weekday: 'narrow' });
            return `<div class="day-label ${i === currentDayIndex ? 'today' : ''}"><span>${dayStr}</span></div>`;
          }).join('')}
        </div>
      </div>
    </div>
  ...
  if (analytics && analytics.hasEnoughData) {
    ...
    <div class="card ${anim2}" style="margin-top: 16px; background: var(--bg-surface-elevated);">
        ...
        <div class="card-title" style="display: flex; align-items: center; gap: 6px;">
          ...
          <div class="info-tooltip-container">
            ...
            <div class="info-tooltip-content">
              Projections are approximate estimates based on your recent usage burn rate and limited data points.
            </div>
          </div>
        </div>
      </div>
      ...
      <div style="display: flex; justify-content: space-between; font-size: 13px; margin-top: 8px;">
        <div style="color: var(--text-secondary);">Recent Usage:</div>
